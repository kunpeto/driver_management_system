# 司機員管理系統

新北捷運輕軌營運處車務中心「司機員事件履歷與考核管理、駕駛時數管理系統」

## 專案狀態

| Phase | 名稱 | 狀態 | 說明 |
|-------|------|------|------|
| **Phase 0** | Setup | ✅ 完成 | 專案基礎架構與開發環境設定 |
| **Phase 2** | Foundational | ✅ 完成 | 核心基礎設施、認證、資料庫 |
| **Phase 3** | US1 - 系統設定 | ✅ 完成 | 部門參數與系統配置 |
| **Phase 4** | US2 - 員工管理 | ✅ 完成 | CRUD、批次匯入/匯出、調動 |
| **Phase 5** | US3 - 權限控制 | ✅ 完成 | RBAC 權限管理 |
| **Phase 6** | US4 - 班表同步 | ✅ 完成 | Google Sheets 整合 |
| **Phase 7** | US6 - 連線監控 | ✅ 完成 | 健康檢查與狀態監控 |
| **Phase 8** | US7 - Google OAuth | ✅ 完成 | OAuth 2.0 授權整合 |
| **Phase 9** | US5 - 駕駛競賽 | ✅ 完成 | 時數統計與競賽排名 |
| **Phase 11** | US8 - 事件履歷 | ✅ 完成 | 履歷管理與 PDF 整合 |
| **Phase 12** | US9 - 考核系統 | ✅ 完成 | 2026 新制考核、R02-R05 雙因子判定 |
| **Phase 13** | US10 - 差勤加分 | ✅ 完成 | 自動分析班表與加分處理 |
| **Phase 14** | US11 - 未結案管理 | ✅ 完成 | 未結案追蹤與 Drive 上傳 |

**最後更新**: 2026-01-31 | **狀態**: 🎉 全功能開發完成

---

## 系統架構

```
┌─────────────────────────────────────────────────────────────────┐
│                      使用者（瀏覽器）                              │
└──────────────┬────────────────────────────┬─────────────────────┘
               │ HTTPS                      │ HTTP (localhost)
               ▼                            ▼
┌─────────────────────────────┐  ┌──────────────────────────────┐
│  GitHub Pages (前端)          │  │  本機桌面應用 API             │
│  - Vue.js 3                  │  │  - FastAPI (localhost:8001) │
└──────────────┬──────────────┘  └──────────────────────────────┘
               │ HTTPS
               ▼
┌─────────────────────────────┐  ┌──────────────────────────────┐
│  Render 雲端 FastAPI          │  │  Google Services             │
│  - RESTful API               │  │  - Sheets (班表/勤務表)       │
│  - JWT 認證                   │  │  - Drive (文件儲存)           │
└──────────────┬──────────────┘  └──────────────────────────────┘
               │
               ▼
┌─────────────────────────────┐
│  TiDB Serverless             │
│  - MySQL 8.0 相容             │
│  - 5GB 免費儲存               │
└─────────────────────────────┘
```

---

## 快速開始

### 環境需求

- Python 3.11+
- Node.js 18+
- Git

### 安裝步驟

```bash
# 1. Clone 專案
git clone https://github.com/kunpeto/driver_management_system.git
cd driver_management_system

# 2. 建立 Python 虛擬環境
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

# 3. 安裝後端依賴
pip install -r requirements.txt

# 4. 複製環境變數範本
cp .env.example .env
# 編輯 .env 填入實際的密碼和憑證

# 5. 測試 TiDB 連線
python test_tidb_connection.py

# 6. 啟動後端開發伺服器
cd backend
uvicorn src.main:app --reload --port 8000

# 7. 安裝前端依賴（另開終端機）
cd frontend
npm install
npm run dev
```

### 訪問服務

| 服務 | URL | 說明 |
|------|-----|------|
| 前端開發 | http://localhost:5173 | Vite 開發伺服器 |
| 後端 API | http://localhost:8000 | FastAPI |
| API 文件 | http://localhost:8000/docs | Swagger UI |
| 健康檢查 | http://localhost:8000/health | 服務狀態 |

---

## 專案結構

```
driver_management_system/
├── backend/                    # 雲端 API（Render 部署）
│   ├── src/
│   │   ├── api/               # API 路由
│   │   ├── config/            # 設定檔
│   │   ├── models/            # 資料模型
│   │   ├── services/          # 業務邏輯
│   │   ├── middleware/        # 中間件
│   │   └── main.py            # FastAPI 入口
│   └── tests/                 # 測試
│
├── frontend/                   # 前端（GitHub Pages 部署）
│   └── src/
│       ├── views/             # 頁面元件
│       ├── components/        # 共用元件
│       ├── stores/            # Pinia 狀態管理
│       └── router/            # Vue Router
│
├── desktop_app/                # 本機 API（檔案處理）
│   └── src/
│
├── specs/                      # 規格文件
│   └── 001-system-architecture/
│       ├── spec.md            # 功能規格
│       ├── plan.md            # 實作計畫
│       └── tasks.md           # 任務分解
│
├── docs/                       # 文件
├── scripts/                    # 腳本工具
├── render.yaml                 # Render 部署配置
└── requirements.txt            # Python 依賴
```

---

## 部署

### 雲端後端（Render）

✅ **已自動部署**

後端透過 Render Blueprint 自動部署：
1. 連接 GitHub Repository 到 Render
2. 選擇 Blueprint 部署（自動讀取 `render.yaml`）
3. 在 Render Dashboard 設定環境變數
4. 每次推送到 `main` 分支自動重新部署

**部署 URL**: https://driver-management-system.onrender.com

---

### 前端（GitHub Pages）

✅ **已配置自動部署**

前端透過 GitHub Actions 自動部署到 GitHub Pages。

#### 一次性設定（必須）

1. 前往 GitHub Repository **Settings** → **Pages**
2. 在 **Source** 選擇：`GitHub Actions`
3. 儲存設定

#### 自動部署

設定完成後，每次推送到 `main` 分支且包含 `frontend/` 目錄變更時，會自動：
- ✅ 建置前端專案
- ✅ 部署到 GitHub Pages

**部署 URL**: https://kunpeto.github.io/driver_management_system/

#### 手動觸發

在 GitHub Repository 的 **Actions** 頁面可手動觸發部署。

詳細設定步驟請參考：[GitHub Pages 部署指南](./docs/github_pages_setup.md)

---

## 開發進度

詳見 [PROGRESS.md](./PROGRESS.md)

### 🎉 全功能開發完成

所有規劃任務皆已完成，系統已可上線使用。

#### 核心功能
- [x] 員工管理（CRUD、批次匯入/匯出、調動記錄）
- [x] 事件履歷管理（調查、訪談、矯正、考核四種子表）
- [x] 2026 新制考核系統（R02-R05 雙因子判定、累計加重）
- [x] 駕駛競賽積分與排名
- [x] 差勤加分自動處理

#### 整合功能
- [x] Google Sheets 班表同步
- [x] Google Drive 文件上傳
- [x] Google OAuth 2.0 授權
- [x] 本機桌面應用 PDF 處理

#### 系統架構
- [x] JWT 認證與 RBAC 權限控制
- [x] API 穩定性契約管理
- [x] Render 雲端部署
- [x] TiDB Serverless 資料庫

---

## 文件索引

| 文件 | 說明 |
|------|------|
| [spec.md](./specs/001-system-architecture/spec.md) | 完整功能規格 |
| [plan.md](./specs/001-system-architecture/plan.md) | 實作計畫 |
| [tasks.md](./specs/001-system-architecture/tasks.md) | 任務分解（131 個任務） |
| [constitution.md](./.specify/memory/constitution.md) | 專案憲章 |
| [data_migration_plan.md](./docs/data_migration_plan.md) | 歷史資料遷移計畫 |
| [API_CONTRACT.md](./docs/API_CONTRACT.md) | API 契約與穩定性規範 |
| [CHANGELOG_API.md](./docs/CHANGELOG_API.md) | API 變更日誌 |
| [github_pages_setup.md](./docs/github_pages_setup.md) | GitHub Pages 自動部署設定 |
| [PROGRESS.md](./PROGRESS.md) | 開發進度追蹤 |
| [TODO.md](./TODO.md) | 待辦事項 |

---

## 技術棧

### 後端
- FastAPI 0.128+
- SQLAlchemy 2.0+
- TiDB (MySQL 8.0)
- APScheduler
- Google API Python Client

### 前端
- Vue.js 3.4+
- Vite
- Pinia
- Vue Router
- Axios

### 部署
- Render (後端)
- GitHub Pages (前端)
- TiDB Serverless (資料庫)

---

## 授權

本專案為新北捷運輕軌營運處車務中心內部系統，僅供內部使用。
