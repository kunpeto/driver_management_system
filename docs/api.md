...
本系統採用嚴格的 API 契約管理機制，確保桌面應用程式（.exe）與雲端後端的相容性。

### 保護等級

| 等級 | 說明 | 修改限制 |
|------|------|----------|
| **CRITICAL** | 絕對不可修改 | 桌面應用 .exe 直接依賴，禁止任何破壞性變更 |
| **STABLE** | 僅可向後兼容修改 | 可新增欄位，不可移除或修改現有欄位 |
| **EVOLVING** | 可在主版本更新時修改 | 需提前通知並記錄變更 |
...
| 端點 | 用途 | 依賴方 |
|------|------|--------|
| `GET /api/settings/value/{key}` | 取得系統設定值 | 桌面應用 |
| `GET /health` | 健康檢查 | 前端 Web 應用 |
| `POST /api/pdf/scan` | 掃描 PDF 條碼 | 前端 Web 應用 |
| `POST /api/pdf/process` | 處理 PDF | 前端 Web 應用 |
| `POST /api/barcode/generate` | 生成條碼 | 前端 Web 應用 |

### 相關文檔

- **[API_CONTRACT.md](./API_CONTRACT.md)** - 完整的 API 契約規範
- **[CHANGELOG_API.md](./CHANGELOG_API.md)** - API 變更日誌
