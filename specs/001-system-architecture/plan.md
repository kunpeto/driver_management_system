...
### Phase 4: 司機員事件履歷管理系統（User Story 8）⭐
- Profile 履歷主表與子表（事件調查、人員訪談、矯正措施、考核通知）
- Office 文件自動生成（Word/Excel + Code128 條碼）
- 履歷類型轉換與狀態管理
- Google Sheets 班表資料整合（自動帶入事件前後班別）
- 履歷查詢與篩選功能

### Phase 5: 考核系統 V2 升級（User Story 9）⭐
- 考核標準表管理（5 類扣分、5 類加分）
- 累計加重機制實作（2026 年起適用）
- 雙版本並存（V1/V2 自動切換）
- 考核記錄管理與年度摘要
...
### Phase 6: 差勤加分自動處理（User Story 10）⭐
- 月度全勤自動判定（+M01）
- R班出勤自動判定（+A01）
- 延長工時自動判定（+A03~+A06）
- 批次建立差勤加分履歷
